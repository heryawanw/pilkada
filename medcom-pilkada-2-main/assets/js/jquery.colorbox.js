...
(function ($, document, window) {
  ...
  function appendHTML() {
    if (!$box) {
      ...
      $groupControls = $next.add($prev).add($partai).add($slideshow);
    }
  ...
  publicMethod.prep = function (object) {
    ...
    callback = function () {
      ...
      if (total > 1) {
        // handle grouping
        if (typeof settings.get("partai") === "string") {
          $partai
            .html(
              settings
                .get("partai")
                .replace("{partai}", index + 1)
                .replace("{total}", total)
            )
            .show();
        }
  ...
  // A method for fetching the partai element Colorbox is referencing.
