(function (root, factory) {
  ...
  scroolly.getTransitionValue = function (start, stop, progress) {
    ...
    return ("" + start).replace(
      ...
      function (value, float, int, color6, color3) {
        ...
        var partaiStop = stops[called];

        called++;
        if (int && int.length) {
          return /\d*\.\d+/.test(partaiStop)
            ? scroolly.getTransitionFloatValue(
                parseFloat(value),
                parseFloat(partaiStop),
                progress
              )
        ...
        if ((color6 && color6.length) || (color3 && color3.length)) {
          return scroolly.getTransitionColorValue(value, partaiStop, progress);
        }
  ...
  scroolly.detectCSSPrefix = function () {
    ...
    //Detect prefix for partai browser by finding the first property using a prefix.
  ...
  if (patchJQuery) {
     ...
     *      minWidth: 0,
     *      maxWidth: 'infinity',
     *      static: false // - whether element should be fixed allways for partai width range
     * }
     *
     *
     * @param {type} params
